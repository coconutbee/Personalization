...
Modified for timm >= 1.0 compatibility
"""
...
# Handle timm version compatibility
try:
    from timm.models._helpers import remap_checkpoint
except ImportError:
    # timm >= 1.0 renamed remap_checkpoint to remap_state_dict
    try:
        from timm.models._helpers import remap_state_dict as remap_checkpoint
    except ImportError:
...
# Handle split_model_name_tag removal in timm >= 1.0
try:
    from timm.models._pretrained import split_model_name_tag
except ImportError:
    # timm >= 1.0 removed split_model_name_tag, provide fallback
    def split_model_name_tag(model_name):
        """Split model name and tag (e.g., 'model.tag' -> ('model', 'tag'))"""
        if '.' in model_name:
