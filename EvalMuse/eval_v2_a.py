...
def smart_find_swapped_image(base_dir, prompt, target_id):
    ...
    # 轉字串並補零 (視你的 ID 格式而定，若無補零需求可移除 zfill)
    ...
    for filename in os.listdir(base_dir):
        if "_" in filename:
            ...
            # 寬鬆檢查：只要 ID 吻合，且 Prompt 吻合
            if str(file_id) != target_id_str and str(file_id) != str(target_id).zfill(5):
                continue
...
def find_target_by_prompt(base_dir, prompt):
    ...
    支援 'prompt.jpg' 與 '00051_prompt.jpg' 兩種格式。
    """
...
# ==========================================
# 4. 核心處理邏輯 (Mode Selection)
# ==========================================
def process_task(task_name, swapped_dir, t2i_dir, json_path, components, mode='full'):
    ...
    for item in tqdm(data, desc=f"   Running {task_name}"):
        ...
        target_id = item.get('id') or item.get('rand_id')
        ...
        # --- A. 計算 Swapped Image 分數 (僅 Full 模式) ---
        if mode == 'full':
            swapped_path = smart_find_swapped_image(swapped_dir, prompt, target_id)
            if swapped_path:
                try:
        ...
        # --- B. 計算 Original T2I Image 分數 (所有模式) ---
    ...
    if missing_t2i > 0:
        print(f"⚠️  Warning: {missing_t2i} T2I images not found.")
    if mode == 'full' and missing_swap > 0:
        print(f"⚠️  Warning: {missing_swap} Swapped images not found.")
    ...
    if mode == 'full':
        result_dict['avg_swap'] = avg(stats['swap'])
        result_dict['count_swap'] = len(stats['swap'])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Batch FGA Evaluation with Mode Selection")
    ...
    parser.add_argument("--t2i", type=str, required=True, help="Directory of T2I original images")
    parser.add_argument("--swap", type=str, default=None, help="Directory of swapped images (Optional if mode=t2i)")
    # 新增 Mode 參數
    parser.add_argument("--mode", type=str, default="full", choices=["full", "t2i"], help="Evaluation Mode: 'full' or 't2i'")
    ...
    if args.mode == 'full':
        ...
        print(f"{'Method':<15} | {'FGA Orig (T2I)':<18} | {'Count':<8}")
        print("-" * 60)
        for res in summary:
            print(f"{res['name']:<15} | {res['avg_orig']:<18.4f} | {res['count_orig']:<8}")
    print("="*80)
