...
# ==========================================
# 3. File Search Utilities (關鍵修正部分)
# ==========================================
...
def smart_find_ref_image(ref_dir, ref_index_str):
    ...
    candidates = [
        ...
        f"0_{ref_index_str}.jpg", f"0_{ref_index_str}.png"
    ]
    ...
    for filename in os.listdir(ref_dir):
        if filename.startswith(f"{ref_index_str}_") or filename.startswith(f"{ref_index_str}."):
             return os.path.join(ref_dir, filename)
             
    return None

# ==========================================
# 4. Core Task Processing (With Mode)
# ==========================================
def process_task(task_name, swapped_dir, t2i_dir, ref_dir, json_path, models, mode='full'):
    ...
    for item in tqdm(data_list, desc=f"   Running {task_name}"):
        ...
        # --- 1. Load Images ---
        # A. Swapped Image (Only in full mode)
        img_swapped = None
        if mode == 'full':
        ...
        # B. T2I Original Image (Required in both modes)
        ...
        # C. Reference Image (Required in both modes for ID metric)
        img_ref = None
        if target_id:
            ref_path = smart_find_ref_image(ref_dir, str(target_id))
            if ref_path:
        ...
        # [Metric 1] Prompt Alignment (Text vs Image)
        if prompt:
            # T2I (Always)
            if img_orig:
        ...
        # [Metric 2] Structure Preservation (Orig vs Swapped) - Full Mode Only
        ...
        if img_ref:
            ...
            # Swapped ID (Ref vs Swapped) - Full Mode Only
            if img_swapped:
                # CLIP ID
    ...
    if missing_t2i > 0:
        print(f"\n⚠️  Warning: {missing_t2i} T2I images were not found.")
        
    if mode == 'full' and missing_swaps:
    ...
    result_dict = {
        ...
        'count': len(stats['t2i_clip_t2i']) # Count based on T2I samples
    }
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ref", type=str, required=True, help="Reference Dir (Required for ID metric)")
    # Mode argument
    ...
    if args.mode == 'full':
        ...
        # T2I Only Output
        print(f"{'Method':<10} | {'T2I(Orig)':<12} | {'C-ID(Orig)':<12} | {'D-ID(Orig)':<12}")
        print("-" * 60)
        for res in results_summary:
            print(f"{res['name']:<10} | "
                  f"{res['t2i_orig']:<12.4f} | "
                  f"{res['t2i_clip_id_i2i']:<12.4f} | "
                  f"{res['t2i_dino_id_i2i']:<12.4f}")
    print("="*145)
